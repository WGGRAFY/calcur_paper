...
get_N_and_indicator <- function(age_comp, fleets){
  ...
  #aggregate CAAL comps into marginal N at a
  if(any(age_comp$Lbin_lo!=(-1))){
    age_comp <- age_comp %>%
      group_by(Yr,Seas,FltSvy,Gender) %>%
      summarise(across(starts_with("f"), sum), Nsamp=Nsamp) %>%
      ungroup()
...
age_comp_catch_combined <- age_comp_catch$age_comp[,10:26] + age_comp_catch$age_comp[,27:43]
names(age_comp_catch_combined) <- gsub(pattern="f", replacement="c", x=names(age_comp_catch_combined))
