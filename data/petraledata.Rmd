...
petrale_data <-r4ss::SS_readdat("./data/2019_petrale.dat")
petrale_control <- r4ss::SS_readctl("./data/2019_petrale.ctl")
...
waa_matrix <- r4ss::SS_readwtatage("./data/wtatage.ss_new")
```
...
a50 <- petrale_control$MG_parms["Mat50%_Fem_GP_1","INIT"]
...
catch_matrix <- petrale_data$catch[,c(1,3,4)] %>% pivot_wider(names_from=fleet, values_from=catch)
catch_se_matrix <- petrale_data$catch[,c(1,3,5)] %>% pivot_wider(names_from=fleet, values_from=catch_se)
...
total_age_comp <- petrale_data$agecomp
...
CPUE_data <- petrale_data$CPUE[,-c(2,5)] %>% pivot_wider(names_from=index,values_from=obs)
CPUE_se_log <- petrale_data$CPUE[,-c(2,4)] %>% pivot_wider(names_from=index,values_from=se_log)
...
7) maturity at age (n_y x n_a) *all maturity by length*
